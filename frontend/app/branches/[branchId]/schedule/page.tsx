        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16, marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 4 }}>
              Simulate customer foot-traffic rushes (e.g. rainy morning, campus festival, or promotional campaigns) to observe live Erlang-C staffing recalculations.
            </div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            >
              Surge: {demandMultiplier.toFixed(2)}x ({demandMultiplier > 1.0 ? `+${Math.round((demandMultiplier - 1.0) * 100)}% Surge` : "Baseline"})
            </span>
          </div>
        </div>
        ...
        >
          {hours.map((h) => {
            ...
            return (
              <div
                key={h.hour}
                title={`${h.label}: ${h.total_orders} Total Orders (${h.avg_orders_per_hr} avg/hr)\nErlang-C Staff Required: ${h.recommended_staff} servers`}
                style={{
                  display: "flex",
