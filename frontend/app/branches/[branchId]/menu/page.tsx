...
export default function BranchMenuPage() {
  ...
  const [form, setForm] = useState({ masterItemId: "", priceOverride: "", isInStock: true, isActive: true });
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => { setForm({ masterItemId: "", priceOverride: "", isInStock: true, isActive: true }); setUpsertModal(true); }}>
      ...
      <div className="card table-wrap">
        {items.length === 0 ? (
          ...
          <table>
            <thead>
              <tr>
                <th>Item</th><th>Base Price</th><th>Override</th><th>Effective Price</th><th>In Stock</th><th>Active</th><th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {items.map(item => (
                <tr key={item.id}>
                  ...
                  <td>
                    <button className="btn btn-ghost btn-sm" onClick={() => { setEditItem(item); setForm({ masterItemId: String(item.masterItem.id), priceOverride: item.priceOverride ? String(item.priceOverride) : "", isInStock: item.isInStock, isActive: item.isActive }); }}>
                      <Pencil size={13} /> Edit
                    </button>
                  </td>
                </tr>
